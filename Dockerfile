...
ENV PATH /opt/conda/envs/NanoR4.1.0/bin:$PATH
SHELL ["/bin/bash", "-c"]
RUN source /opt/conda/etc/profile.d/conda.sh && conda activate NanoR4.1.0 && R -e 'install.packages("devtools", repos = "http://cran.rstudio.com/")' && R -e 'devtools::install_github("jmw86069/splicejam", dependencies = TRUE)'
